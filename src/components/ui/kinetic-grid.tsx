...
export default function KineticGrid({
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      const LINE_BASE = isLight ? { r: 0, g: 0, b: 0, a: 0.13 } : { r: 255, g: 255, b: 255, a: 0.13 };
      ...
      const currentTheme = {
        default: {
          bg: "transparent",
          // Dark green instead of bright green for grid hovers
          lineActive: { r: 5, g: 80, b: 20, a: 0.9 }, 
          nodeActive: { r: 5, g: 80, b: 20, a: 1.0 },
          glow: "5,80,20",
          ripple: "5,80,20",
        },
        monochrome: {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          if (t > 0.3) {
            ...
            grd.addColorStop(0, `rgba(${currentTheme.glow},${(t * 0.3).toFixed(3)})`);
          ...
          ctx.fillStyle = lerpColor(
            isLight ? { r: 0, g: 0, b: 0, a: 0.1 } : { r: 255, g: 255, b: 255, a: 0.2 },
            currentTheme.nodeActive,
            t,
          );
          ctx.fill();
        }
      ...
      for (const r of ripples) {
        ...
        ctx.strokeStyle = `rgba(${currentTheme.ripple},${(r.opacity * 0.28).toFixed(3)})`;
        ctx.lineWidth = 1.5;
        ctx.stroke();
      }
    },
    [getWarpedPoint, globalColor, theme],
  );
